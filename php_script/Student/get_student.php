...
{   
  ...
  if(isset($_GET['id']) && $_GET['id']!=null)
  {
      ...
      $choose = Trans::getStudentChooseByIdStudent($student->id);//получаем его выбранное направление и кафедру
      $transfers = Trans::getTransfersByIdDirection($choose['id_direction']);//плучаем переход для него для этого направления
      
      //вывод информации о студенте
      ?>
<div>
  <div id="stud-info">    Фамилия: <?= $student->last_name ?>
    Имя: <?= $student->name ?>
    Отчество: <?= $student->last_name ?>
    ...
    <div id="stud-predmet">
        <table class="table-bordered">
            ...
            <tbody>
            <?php foreach ($transfers as $transfer) //формируем таблицу соотсвествия предметов и дисциплин
                  {
            ?>
                <tr>
